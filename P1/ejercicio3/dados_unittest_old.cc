...
// Test Lanzamiento 1 y 2, y lanzamientos n
TEST(Dados, Lanzamientos){
	...
	EXPECT_EQ(20000, e.getLanzamientos()+e.getLanzamientos1());
...
TEST(Dados, Media){
	...
	EXPECT_EQ(d.getMedia1(), 0);
	EXPECT_EQ(d.getMedia2(), 0);
	...
	EXPECT_GT(e.getMedia1(), a);
	EXPECT_GT(e.getMedia2(), b);
}
...
TEST(Dados, Vector){
	...
	for(int i=0; i<1000; i++){
		d.lanzamiento();
		d.almacenad1(d.getDado1());
		d.almacenad2(d.getDado2());		

		EXPECT_EQ(d.getDado1(), d.vdatos1(0));
		EXPECT_EQ(d.getDado2(), d.vdatos2(0));
	}	
}
